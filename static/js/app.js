...
window.App = {
  state: {
    ...
    period: 'all',
    activeView: 'overview',
    yoyEnabled: true,  // YoY default ON per spec
    selectedMetrics: ['sessions', 'conversions'],  // checked metric pills
    trendlineMetric: 'sessions',
...
function bindFilters() {
  ...
  // Metric checkboxes (top of main chart)
  qsa('.metric-pill input[type="checkbox"]').forEach(cb => {
    cb.addEventListener('change', () => {
      const m = cb.dataset.metric;
      if (cb.checked) {
        if (!App.state.selectedMetrics.includes(m)) App.state.selectedMetrics.push(m);
      } else {
  ...
  // Trendline metric selector
  byId('trendline-metric')?.addEventListener('change', e => {
    App.state.trendlineMetric = e.target.value;
    renderMainChart();
  });
...
async function loadStatus() {
  ...
  const dst = byId('data-status');
  if (dst) dst.innerHTML = (s.data_range && s.data_range.rows)
...
const METRIC_META = {
  sessions: { label: 'Návštěvnost', color: '#FF6B35', bg: 'rgba(255,107,53,0.12)', axis: 'y' },
  conversions: { label: 'Konverze', color: '#22c55e', bg: 'rgba(34,197,94,0.12)', axis: 'y' },
  conv_rate: { label: 'Konv. míra (%)', color: '#a855f7', bg: 'rgba(168,85,247,0.12)', axis: 'y2' },
};

async function renderKpiRow() {
  const ids = effectivePropertyIds();
  const wrap = byId('kpi-row');
  if (!wrap) return;
  if (!ids.length || App.state.selectedMetrics.length === 0) {
    wrap.innerHTML = '';
    return;
  }
  const { start, end } = periodRange();
  const params = `property_ids=${ids.join(',')}&metrics=${App.state.selectedMetrics.join(',')}${start ? '&start='+start : ''}${end ? '&end='+end : ''}${App.state.yoyEnabled ? '&yoy=true' : ''}`;
  const data = await api(`/api/data/aggregate?${params}`);

  wrap.innerHTML = App.state.selectedMetrics.map(m => {
    const meta = METRIC_META[m];
    const total = data.totals[m];
    const yoy = data.yoy_totals?.[m];
    let yoyHTML = '';
    if (yoy != null && yoy !== 0) {
      let pct;
      if (m === 'conv_rate') {
        ...
        const sign = pct >= 0 ? '+' : '';
        yoyHTML = `<div class="yoy ${cls}">${sign}${pct.toFixed(2)} pp YoY (loni ${yoy}%)</div>`;
      } else {
        pct = (total - yoy) / yoy * 100;
        const cls = pct > 1 ? 'up' : pct < -1 ? 'down' : 'flat';
        yoyHTML = `<div class="yoy ${cls}">${fmtPct(pct)} YoY (loni ${fmt(yoy)})</div>`;
      }
    }
    const valDisplay = m === 'conv_rate' ? `${total}%` : fmt(total);
    return `<div class="kpi-card" style="border-left: 3px solid ${meta.color}">
      <div class="label">${meta.label}</div>
      <div class="value" style="color:${meta.color}">${valDisplay}</div>
      ${yoyHTML}
    </div>`;
  }).join('');
}
...
async function renderMainChart() {
  ...
  const params = `property_ids=${ids.join(',')}&metrics=${App.state.selectedMetrics.join(',')}${start ? '&start='+start : ''}${end ? '&end='+end : ''}${yoy}`;
  const data = await api(`/api/data/aggregate?${params}`);

  const datasets = [];
  // For each selected metric, add main line + optional YoY dashed
  App.state.selectedMetrics.forEach(m => {
    const meta = METRIC_META[m];
    const series = data.series[m] || [];
    if (!series.length) return;
    datasets.push({
      label: meta.label,
      data: series.map(d => ({ x: d.week_start, y: d.value })),
      borderColor: meta.color,
      backgroundColor: meta.bg,
      borderWidth: 2.5, fill: false, pointRadius: 0, tension: 0.3,
      yAxisID: meta.axis,
      order: 1,
    });
    if (data.yoy_series && data.yoy_series[m]) {
      datasets.push({
        label: `↶ ${meta.label} loni`,
        data: data.yoy_series[m].map(d => ({ x: d.week_start, y: d.value })),
        borderColor: '#94a3b8',  // šedá pre loňský rok — jasne odlíšená od current
        backgroundColor: 'rgba(148,163,184,0.05)',
        borderDash: [3, 3],
        borderWidth: 1.5, fill: false, pointRadius: 0, tension: 0.3,
        yAxisID: meta.axis,
        order: 3,
      });
    }
  ...
  const usesY2 = App.state.selectedMetrics.some(m => METRIC_META[m].axis === 'y2');
  const usesY = App.state.selectedMetrics.some(m => METRIC_META[m].axis === 'y');

  const scales = {
    x: { type: 'time', time: { unit: 'week' }, ticks: { color: '#94a3b8' }, grid: { color: '#1f2a3a' } },
  };
  if (usesY) scales.y = { type: 'linear', position: 'left', ticks: { color: '#94a3b8' }, grid: { color: '#1f2a3a' }, beginAtZero: true };
  if (usesY2) scales.y2 = { type: 'linear', position: 'right', ticks: { color: '#a855f7', callback: v => v + '%' }, grid: { drawOnChartArea: false }, beginAtZero: true };
  ...
  App.charts.main = new Chart(canvas.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        legend: { position: 'bottom', labels: { color: '#e2e8f0', boxWidth: 12, font: { size: 12 }, padding: 12 } },
        tooltip: {
          backgroundColor: '#1a2230', borderColor: '#FF6B35', borderWidth: 1, padding: 12,
          titleFont: { size: 13 }, bodyFont: { size: 12 },
          callbacks: {
            label: (ctx) => {
              const v = ctx.parsed.y;
              const m = App.state.selectedMetrics.find(mm => METRIC_META[mm].label === ctx.dataset.label || ctx.dataset.label?.startsWith(METRIC_META[mm].label));
              const isPct = ctx.dataset.yAxisID === 'y2';
              return `${ctx.dataset.label}: ${isPct ? v.toFixed(2) + '%' : v.toLocaleString('cs-CZ')}`;
            },
          },
        },
      },
